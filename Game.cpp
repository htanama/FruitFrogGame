...
void Game::Start(const char* title, int xpos, int ypos, int width, int height)
{
	...
	mGameWindow = SDL_CreateWindow("Fruit Frog", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
	...
	m_isRunning = true;
	frog = new Player("assets/fruitfrogspritesheets.png", 0, 700);
...
void Game::WaiveOf_FallingObject()
{
	...
	if (tmpRect.y > 760 || wood1->isCollide())
	{
	...
	randomX = (rand() % 900) + 1;
	if (tmpRect.y > 760 || wood2->isCollide())
	{
	...
	randomX = (rand() % 900) + 1;
	if (tmpRect.y > 760 || wood3->isCollide())
	{
		std::cout << "Wood2 Out from the screen\n";
		delete wood3;
		wood3 = NULL;
	...
	randomX = (rand() % 900) + 1;
	if (tmpRect.y > 760 || wood4->isCollide())
	{
		std::cout << "Wood2 Out from the screen\n";
		delete wood4;
		wood4 = NULL;
	...
	if (tmpRect.y > 760 || apple->isCollide())
	{
	...
	if (tmpRect.y > 760 || banana->isCollide())
	{
	...
	if (tmpRect.y > 760 || strawberry->isCollide())
	{
	...
	if (tmpRect.y > 760 || grape1->isCollide())
	{
	...
	if (tmpRect.y > 760 || grape2->isCollide())
	{
...
void Game::update()
{
	if (isGamePause) // default value of isGamePause = false
	{
		GameIsPauseMessage();		
	}
	else
